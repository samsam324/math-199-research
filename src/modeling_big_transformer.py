"""
Larger transformer architecture with proper training regime, decoupled from
the small reference transformer in src/modeling.py so the comparison is
clean. Used to test whether the negative transformer result in the original
walk-forward evaluation is a model-capacity / training-budget artifact.

Architecture: 4-layer encoder, 4 heads, d_model=128, ff=512, dropout=0.2,
sinusoidal positional encoding, CLS token pooling.
Training: AdamW, cosine LR schedule with 1-epoch warmup, weight decay 1e-4,
gradient clipping at 1.0, validation-based early stopping on a 10% tail of
the training window with patience 5.
"""
...
def train_big_transformer_on_split(
    ...
    # Validation slice = last val_fraction of training (time-ordered)
    ...
    for epoch in range(cfg.epochs):
        ...
        with torch.no_grad():
            for xb, yb in val_loader:
                xb, yb = xb.to(device), yb.to(device)
                logits = model(xb)
                val_loss += float(loss_fn(logits, yb)) * len(yb)
                n += len(yb)
        val_loss /= max(1, n)
    ...
    base_cfg = TrainingConfig(seed=cfg.seed)
    return _metrics_for_predictions("big_transformer", train, test, pred)


def train_big_transformer(
    samples: pd.DataFrame,
    sequences: np.ndarray,
    cfg: BigTransformerConfig = BigTransformerConfig(),
    base_cfg: TrainingConfig = TrainingConfig(),
) -> Tuple[pd.DataFrame, Dict[str, float]]:
    """Single-split version for parity with src/modeling.train_transformer."""
    ...
    return train_big_transformer_on_split(train, test, sequences, cfg)
