...
errors~\cite{newey1987hac}, because overlapping
...
them~\cite{saiddickey1984}. Under this screen $100\%$ of the top 50 pairs rejected the unit root out of sample at
...
\begin{figure}[t]
\centering
\figplaceholder{fig\_kalman\_innovations\_white.pdf}{scratch/kalman\_positive\_control.py (re-run to dump innovations)}
...
\begin{figure}[t]
\centering
\figplaceholder{fig\_poscontrol\_vs\_negcontrol.pdf}{scratch/kalman\_positive\_control.csv}
...
\begin{figure}[t]
\centering
\figplaceholder{fig\_freq\_invariance.pdf (RESHAPED)}{table values reshaped to show placebo gap}
...
frequency dependent. This reshaped view makes visible what
Table~\ref{tab:kalman-freq}'s percentages only imply: a flat zero line is the diagnostic. (Note:
this replaces the original bar-chart version that simply duplicated the table.)}
\label{fig:freq}
\end{figure}
...
\begin{figure}[t]
\centering
\figplaceholder{fig\_reversion\_persistence\_scatter.pdf}{scratch/persistence\_pairs.csv}
...
$2.6\times$ retail, the longer horizon permanent estimate is noisier;
Table~\ref{tab:institutional-impact}). Book side order flow
imbalance~\cite{contkukanovstoikov2014} explains subsecond midprice moves better than trade flow does (incremental $R^2$
...
\begin{table}[h]
\centering
\caption{[PLACEHOLDER --- to be populated from \texttt{scratch/impact\_decomp\_2024.log}.]
Per-trade price impact at $1$-second horizon by trade size bucket and symbol: institutional
($>$\$$10$k) consistently $2.0$--$2.6\times$ retail across BTC/ETH/SOL/AVAX, validating
...
\begin{figure}[t]
\centering
\figplaceholder{fig\_ofi\_decay.pdf}{re-run scratch/book\_ofi\_incremental.py with horizons [1,2,5,10,30,60,120,300] s --- requires L2 data on Windows}
\caption{Predictive incremental $R^2$ of book-OFI for forward midprice moves decays from
$\sim$$10$--$15\%$ contemporaneously to near zero by $30$ seconds, across BTC/ETH/SOL/AVAX
...
\begin{figure}[t]
\centering
\figplaceholder{fig\_cancellation\_share.pdf}{scratch/book\_ofi\_2024.log cancel-share fields --- requires L2 data on Windows}
\caption{Cancellations dominate trades in best-level liquidity withdrawal: $81$--$85\%$ of
...
\begin{table}[h]
\centering
\caption{[PLACEHOLDER --- to be populated from \texttt{scratch/exec\_value\_2024\_summary.csv}.]
Per-symbol breakdown of execution costs at \$$10$k and \$$50$k notional. Aggressive crossing
dominates across all four majors and both sizes; the L3-from-L2 signal never beats the
random-rate placebo at any cell.}
...
\begin{table}[h]
\centering
\caption{[PLACEHOLDER --- to be populated from \texttt{docs/hac\_sharpe\_fixed.csv} and
\texttt{docs/hac\_sharpe\_fixed\_deep.csv}.] Naive vs HAC-adjusted monthly Sharpe on
...
\begin{tabular}{lrr}
\toprule
Series & Naive monthly Sharpe & HAC-adjusted (lag $240$) \\
\midrule
Binance.US, hourly returns & $2.18$ & $1.40$ \\
Coinbase, hourly returns   & $0.88$ & $0.88$ \\
\midrule
HAC- and venue-centered    & --- & $\approx 1.0$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[t]
\centering
\figplaceholder{fig\_hac\_inflation.pdf}{docs/hac\_sharpe\_per\_split.csv}
\caption{Hourly Sharpe is heavily serial-correlation inflated by multi-week holds. Left: per-split
IID vs Newey-West HAC ratios cluster around $1.5$--$2.5\times$ across the $27$ walk-forward
...
\begin{figure}[t]
\centering
\figplaceholder{fig\_microcap\_adv.pdf}{scratch/microcap\_adv.csv}
\caption{Median daily USD volume on Binance.US, log scale. The preregistered held-out microcaps
(WAXP, ZIL, VTHO, ACH, EGLD; red) trade at hundreds to a few thousand dollars per day, three to
...
\begin{figure}[t]
\centering
\figplaceholder{fig\_forced\_collapse\_perpair.pdf}{scratch/forced\_collapse.csv}
...
\begin{figure}[t]
\centering
\figplaceholder{fig\_circuit\_breaker\_retention.pdf}{scratch/survivorship\_adjusted\_sharpe.csv}
\caption{Monthly Sharpe of the no-stop reversion book as a function of injected delisting rate,
with and without the structural-break circuit breaker. Without the breaker, the Sharpe collapses
past a $10\%$/year break rate. With the breaker, it retains $\approx 96\%$ of its Sharpe at the
