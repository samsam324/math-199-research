...
Sharpe near $1.0$, and only as a never-stop, multi-week hold. We read the result
...
random-walk placebos pass at the same near-$100\%$ rate at hourly, four-hour, and daily bars,
while a clean Engle--Granger test stays far lower. Exact values in
...
($+0.5$ to $+2.6z$) across lookback, horizon, and threshold, and the Kalman placebo passes at
...
marginal reversion edge of Section~\ref{sec:survivor} lives are not in this sample. So the two
findings fit together: the majors are efficient, and the names with an apparent edge are too thin
to trade.
...
overridden). Without a stop the loss runs $-120\%$ to $-260\%$ per pair, with intra-collapse
drawdowns to $-1{,}300\%$; the structural-break circuit breaker (halt after a single leg moves more
than $50\%$ against the position) caps the loss to about $-65\%$ to $-90\%$ and the worst drawdown
to $-120\%$. The breaker blunts the tail but does not remove it, and a fully point-in-time universe
of every delisted coin is still beyond our data.
...
loss runs $-120\%$ to $-260\%$ per pair and the circuit breaker caps it near $-65\%$ to $-90\%$. A
...
The screens were the problem, not the market. The Kalman cointegration test and the rolling
