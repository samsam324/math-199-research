...
Sharpe near $1.0$, but only without a stop and only as a multi-week hold. That $1.0$ is an
exploratory, post-hoc estimate that was never pre-registered, and our pre-registered test gives a
confidence interval spanning zero, so we claim no significant tradable edge. We read the result
as a market-efficiency statement about the liquid core of crypto pair spreads, and as a cautionary
standard: a
...
perfect-foresight oracle would save 1.6 bps, so the opportunity is real, but no contemporaneous book
...
bounded delisting risk from an unbounded one. We also inject permanent delisting-scale breaks into a
fraction of the 40 pairs each quarter. Without the breaker the no-stop book's monthly Sharpe falls
...
$19\%$/yr. Three things make those retention figures optimistic. First, the injected breaks ramp
...
breaks are drawn independently, which understates clustered risk-off waves. The robust finding is the
qualitative one: a leg-crash breaker, not diversification alone, bounds the delisting tail, and at
the low single-digit yearly attrition that liquid top-50 names actually see the haircut is modest. A
