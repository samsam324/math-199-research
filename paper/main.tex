...
Our screens come from the pairs trading and statistical arbitrage literature, in the tradition of
Gatev, Goetzmann, and Rouwenhorst~\cite{gatev2006pairs} and built on Engle-Granger
cointegration~\cite{engle1987cointegration}. A Kalman dynamic hedge, a rolling $z$ score, and the
order flow and price impact measures of Section~\ref{sec:real} are all standard tools, so we do not
reintroduce them here. The contribution is a placebo that two of these standard screens fail.

The closest prior on crypto pairs trading, Fil and Kristoufek~\cite{fil2020pairs}, reports
strategies that underperform their benchmarks and are highly cost sensitive; our results agree and
sharpen the mechanism, since the apparent edge is either a screening artifact or priced within
seconds. The nearest methodological relative is the backtest overfitting work behind the deflated
Sharpe ratio~\cite{bailey2014dsr}: a handful of trials manufactures spurious in sample performance,
so a backtest has to be discounted for the number of tries behind it. That work checks the strategy
you select. We apply the same skepticism one level earlier, to the screen that builds the spread.
