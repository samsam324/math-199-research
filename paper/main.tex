...
Sharpe near $1.0$, but only without a stop and only as a multiweek hold. That $1.0$ is an
...
with a continuously traded market and a fast changing set of listed coins. Our proposal was noncommittal about profit from the start. It framed the goal as quantifying whether stable mean reverting
...
every liquid pair, but it tests a filter's own innovations, which are nearly white by construction.
...
as a patient multiweek hold, and a preregistered held out backtest of it inflates to a Sharpe
near $5$ on microcap moves no one could trade, so even the strategy that survives makes our point.
...
Three rules hold throughout. Inference uses Newey and West HAC standard errors, because overlapping
...
the one correction we made before running are in the repository's commit history.
...
prediction errors, became the spread, and we ran an augmented Dickey and Fuller (ADF) test on
...
It is decisive about the filter, not the pair. A Kalman filter with nonzero process noise
...
and approximately so here on the frozen parameter test window. An ADF test on a nearly white
...
test tells the opposite story. Engle and Granger~\cite{engle1987cointegration} on the test window, and a
...
claiming that no pair ever comoves, only that this screen cannot detect it.
...
test window innovations are white, with a Ljung and Box test failing to reject whiteness for $87\%$ of
...
than the negative ($17\%$ versus $2\%$ for Engle and Granger, $27\%$ versus $8\%$ for static OLS),
...
Kalman innovations white (Ljung and Box $p>0.05$) & $86.7\%$ & $95.0\%$ \\
Static OLS residual white & $0.0\%$ & $0.0\%$ \\
Clean Engle and Granger passes & $16.7\%$ & $1.7\%$ \\
...
Engle and Granger runs at $25\%$ on this long hourly window and falls to the $5\%$ null floor at
...
the screen together at every cadence. The clean Engle and Granger and static OLS benchmarks do
...
Frequency & $p<0.05$ & $p<0.001$ & $p<0.05$ & $p<0.001$ & Engle and Granger & static OLS \\
\midrule
...
daily, where both fall to about $70\%$ under the stricter threshold), while a clean Engle and Granger
...
imbalance explains subsecond midprice moves better than trade flow does (incremental $R^2$
...
autocorrelated, so a Newey and West correction drops the monthly Binance number to about $1.4$, and
...
high volatility microcap alts: individual pair windows post $+300$ to $+520\%$ in three months
...
the held out microcaps (WAXP, ZIL, VTHO, ACH, EGLD) turn over a median of a few hundred to a few
...
The preregistration had a second arm, which we report in full. A single selection over the whole
...
1.89]$ that spans zero. So the preregistered test confirms the inflation and gives no significant
...
overridden). Without a stop the loss runs $-117\%$ to $-258\%$ per pair, with intracollapse
...
and impact aware cost model, and a single preregistered configuration evaluated once on data the
