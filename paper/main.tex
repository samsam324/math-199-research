...
honest accounting posts a HAC and venue adjusted monthly Sharpe near $1.0$, and only without a
stop and only as a multiweek hold. That $1.0$ is an
...
liquid core of crypto pair spreads is efficient. The methodological takeaway is simpler: do not
trust a cointegration or $z$ score screen until you have shown it fails on a matched placebo.
\end{abstract}
...
four hour, and daily frequencies. A rolling $z$ score reports strong mean reversion, yet it reverts
mechanically even on a pure random walk. A single matched placebo exposes each one, and that
placebo is the main thing we contribute. Second, the structure that survives is not tradable: order flow in the Level 2 book
...
near $5$ on microcap moves no one could trade, so even our one surviving strategy ends up cutting
against itself.

The lesson is narrow and reusable: before you believe a cointegration or mean reversion screen,
check that it fails on a matched placebo. The backtest overfitting
literature~\cite{bailey2014dsr, harvey2016crosssection} stress tests the strategy you finally pick;
...
Granger~\cite{engle1987cointegration}. The contribution is a placebo that two standard screens fail.
...
data contingent choices inflate error even without explicit fishing. We apply the same skepticism one level earlier, to the screen that builds the spread.
...
A few conventions hold throughout. Inference uses Newey and West HAC standard errors, because overlapping
...
$19\%$/yr. A few things make those retention figures optimistic. First, the injected breaks ramp
...
The takeaway is a methodological habit more than a tradable strategy. Trust a cointegration or
