...
the L2 book decays within seconds. Reversion speed ranks pairs (Spearman $\rho = 0.46$), but
...
liquid core of crypto pair spreads is efficient. The methodological lesson is simpler: do not
...
(USD quoted) for the overlapping majors. The microstructure work uses event-level L2 data
...
and positive control (Table~\ref{tab:poscontrol}) are consistent with it. A formal transfer
through the frozen forward-rolled filter's steady-state Kalman gain, with Said-Dickey
regularity verified on the resulting innovations, is left for future work.

\emph{Diagnostic note on $q_\beta$.} The fitted process variance is small (median
$q_\beta \approx 2 \times 10^{-9}$ on the positive control). Even at very small
$q_\beta > 0$ the filter operates in the steady-state-whitening
regime~\cite{harvey1989structural}, so the absorption mechanism still applies, and the
placebo pass rate is the same at every $q_\beta$ we observed.
...
We tested the order flow ideas on a 2024 sample of Binance L2 book and trade data: aggregate
...
The efficiency reading is scoped to the liquid core. The L2 tests cover top 50 aggregate book
...
stands.
\item \emph{Microstructure coverage.} The event-level L2 tests use four symbols (BTC, ETH,
...
this paper were produced by \texttt{scratch/make\_figures\_v2.py}. The L2 dataset
(Tardis Binance.com 2024) is in S3; query helpers are in
