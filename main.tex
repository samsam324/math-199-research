...
autocorrelation, and Ero\u{g}lu, Miller, and Yi\u{g}it~\cite{eroglu2022tvc} show that
time-varying-parameter Kalman models are vulnerable to spurious regression because the
integrated error is transferred to the estimated state equation. Our contribution is the
screen-level placebo audit of the specific Kalman dynamic hedge plus ADF pipeline that
circulates in pairs trading practice (\cite{vidyamurthy2004pairs, elliott2005pairs,
triantafyllopoulos2011dynamic}), with a positive control showing the screen cannot
separate constructed cointegrated pairs from random walks. Second, we measure whether L2
...
Three things in this paper are not in the literatures above. First, the screen-level
placebo audit of the Kalman dynamic hedge plus ADF cointegration pipeline. The mechanism
that drives our finding (QMLE Kalman absorbing integrated error into the time-varying
...
screen-level placebo audit of a known induced-autocorrelation artifact.
...
\begin{quote}
\textbf{Proposition (screen-level corollary of \cite{eroglu2022tvc}).}
\emph{Let $y_t$ and $x_t$ be $I(1)$ processes, cointegrated or not. Fit a Kalman
dynamic-hedge model $y_t = \alpha_t + \beta_t x_t + v_t$ with non-degenerate state-evolution
covariance $Q$ by quasi-maximum likelihood on a training window, then forward-roll the
filter with frozen parameters on a test window of length $T$. Then the test-window
one-step innovations $\{e_t\}$ are asymptotically stationary, and an augmented
Dickey-Fuller test on $\{e_t\}$ rejects the unit root with probability tending to $1$
as $T \to \infty$.}

\emph{Sketch.} The QMLE pseudo-true $(Q^\star, R^\star)$ drives the recursive state
updates to absorb the integrated component of $y_t - \beta_t x_t$ into the state estimate
$\hat\beta_t$~\cite{eroglu2022tvc, harvey1989structural}, leaving stationary
innovations. Standard ADF asymptotics~\cite{saiddickey1984} then give rejection
probability $\to 1$ against the stationary alternative.
\end{quote}

The proposition makes precise why the screen carries no cointegration information. The
empirical demonstration is below.
