...
For the avoidance of doubt: our three contributions are bounded as follows. The Kalman screen
...
that drives our finding (quasi-maximum likelihood, QMLE, Kalman absorbing integrated error into the time-varying
...
filter with frozen parameters on a test window of length $T$. Then at the QMLE
pseudo-true parameters the integrated component of $y_t - \beta_t x_t$ is absorbed into
the state estimate $\hat\beta_t$, so the one-step innovations $\{e_t\}$ inherit no
integrated component, and an augmented Dickey-Fuller test on $\{e_t\}$ has asymptotic
power approaching $1$ against the unit-root null as $T \to \infty$.}
...
to $-117\%$. The $50\%$ threshold is the tightest level at which monthly Sharpe is essentially
unchanged from the no-breaker baseline: across a sweep $K \in \{25, 30, 40, 50, 60, 70, 80, 90\}\%$,
