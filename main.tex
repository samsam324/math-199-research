...
Two screens that appear to find strong, tradable structure are screen artifacts, not features of the data. A
...
the useful finding is why: the two screens that made the strategy look best are screen artifacts,
and whatever structure is genuinely present is either priced within seconds or too thin to trade
once costs and selection are counted.
...
Across all three the lens is the same: the screen that produced the candidate is the part to
stress, not the candidate itself. The backtest-overfitting literature~\cite{bailey2014dsr}
stress tests the strategy you finally pick; the screen you used to build the spread needs the
same scrutiny.
...
Wu~\cite{liu2022common}) with a leg-level break-aware stop (Lu et
al.~\cite{lu2021structural}) and asks the joint question for crypto pair spreads. None of
...
that drives our finding, a quasi-maximum-likelihood (QMLE) fit Kalman filter absorbing integrated error into the
time-varying state estimate, is documented at the cointegrating-vector level by Ero\u{g}lu, Miller,
...
than $50\%$ adversely (break-aware stops on equity pair spreads appear in Lu et
al.~\cite{lu2021structural}). The breaker preserves roughly $96\%$ of monthly Sharpe at an
...
Two screens drove our early results, and both are artifacts of the screen, not of the data. The first, a Kalman dynamic
...
is a screen-level analogue of the spurious-regression-in-Kalman-TVP mechanism documented by
...
$T$. We conjecture that the standard ADF test, applied to the one-step innovations $\{e_t\}$
with conventional Said-Dickey critical values, rejects the unit-root null with probability
approaching $1$ as $T$ grows, regardless of whether $y_t$ and $x_t$ are cointegrated. Two
mechanisms are consistent with this: (i) the innovations inherit no integrated component
and the rejection is a correctly-sized power result against an $I(0)$ alternative; (ii) the
innovations behave as static-OLS-style residuals whose null distribution is
Phillips-Ouliaris~\cite{phillipsouliaris1990}, not Said-Dickey, in which case Said-Dickey
critical values are simply the wrong reference under the unit-root null. We do not separate
the two empirically. Either way, the screen does not discriminate cointegrated pairs from
independent random walks.
\end{quote}

We do not prove this. The reasoning under (i) is by analogy: Ero\u{g}lu, Miller, and
...
analogue, and our placebos (Table~\ref{tab:kalman-placebo}, Table~\ref{tab:kalman-anchors})
and positive control (Table~\ref{tab:poscontrol}) are consistent with it. Reading (ii) is
older and more conservative: residual-based unit-root tests on regression residuals have a
non-Dickey-Fuller null distribution~\cite{phillipsouliaris1990}, and small-sample pile-up
of $q_\beta$ at the zero boundary~\cite{stock1994unit} can push the QMLE-frozen filter
into exactly the static-residual regime where Said-Dickey critical values misread the null.
Either reading is consistent with our placebo evidence; what the empirical demonstration
carries is the screen-level claim, not the mechanism. A formal transfer through the frozen
forward-rolled filter's steady-state Kalman gain, with Said-Dickey regularity verified on
the resulting innovations, is left for future work.
...
subsample), $t_0 =$
2024-01-01, 90 day train / 30 day test. The $2$ to $3\%$ clean-test rate is at or below the
nominal $5\%$ test size, consistent with the null of no cointegration on this matched window.}
...
quote is cancellations (under a multi-day size-weighted denominator; see Figure~\ref{fig:cancel-share}
caption), invisible to trade-only analysis.
...
The $\sim$$1.0$ centered figure is a venue-midpoint heuristic, not a properly HAC-adjusted
cross-venue estimate.}
...
we find is not statistically distinguishable from zero on the preregistered test (CI spans zero);
even our exploratory point estimate near Sharpe $1.0$ is marginal, stop sensitive, and selection
...
\bibitem{phillipsouliaris1990}
Phillips, P.~C.~B., \& Ouliaris, S. (1990). Asymptotic properties of residual based tests
for cointegration. \emph{Econometrica}, 58(1), 165--193.
...
\bibitem{stock1994unit}
Stock, J.~H. (1994). Unit roots, structural breaks and trends. In R.~F. Engle \& D.~L.
McFadden (Eds.), \emph{Handbook of Econometrics}, Vol.~4, Ch.~46 (pp.~2739--2841).
Elsevier.
