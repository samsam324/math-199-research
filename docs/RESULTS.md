...
### HAC-corrected variant

The `pnl_mean_to_std` metric uses the standard deviation of per-bar
`signal * change_24h` as its denominator. Consecutive bars have 23 of 24
target hours in common, so the samples are not iid and the sample std
under-estimates the true long-run std of returns. A Newey-West HAC
estimator with bandwidth 24 (matching the target horizon) gives a
consistent long-run-variance estimate. Per-split means averaged across
the 27 splits:

| Model | iid pnl_mean_to_std | HAC pnl_mean_to_std (lag 24) | inflation factor |
| --- | ---: | ---: | ---: |
...
The iid version was over-stating the metric by roughly 1.7x on the
trading-aware models. The HAC correction cuts all of them by about that
factor; the ordering is unchanged. `random_stratified` has inflation
factor 1.00 as a sanity check — random predictions produce uncorrelated
PnL, so iid and HAC agree.

Take the HAC-corrected column as the defensible Sharpe-like statistic.
The iid column is reported above for backwards comparability with
`docs/long_pipeline_results.md` and the prior pipeline runs.

Source: `docs/hac_sharpe_per_split.csv`, generated by
`scripts/run_hac_sharpe.py`.
