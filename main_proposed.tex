...
\title{Screening artifacts in cryptocurrency pair spreads:\\
why Kalman cointegration tests pass random walks,\\
and what survives once they don't}
\author{Jack Lutz \and Sammy Adham \and Frank Kronewitter}
\date{June 21, 2026\\[0.4em]\small Advisor: Mihai Cucuringu}
...
pair spreads. The apparent profits came from two statistical artifacts. A
...
the Level 2 book decays within seconds, and that null holds across all fifty of the most liquid symbols. Reversion speed ranks pairs (Spearman $\rho = 0.46$), but
...
confidence interval spanning zero, so we claim no significant tradable edge. This is evidence that
the liquid core of crypto pair spreads is efficient. A cointegration or $z$ score screen should be
trusted only after it fails on a matched placebo.
\end{abstract}
...
We began with an hourly crypto-pairs backtest and found that its strongest signals did not
survive placebo tests. The two screens that made the strategy look best are statistical
artifacts, and whatever structure is genuinely present is either priced within seconds or too
thin to trade once costs and selection are counted.
...
spread when it deviates, and collect the reversion. Crypto markets trade continuously and list
coins rapidly, so they are a useful setting for testing whether stable mean-reverting relationships
exist. Our proposal was noncommittal about profit from the start. It framed the goal as
quantifying those relationships, ``as a way of gaining a deeper understanding of market efficiency
and information flow.'' The answer is mostly negative.

The paper makes three claims. First, two standard pair screening tools manufacture the
appearance of structure on data that cannot carry it. A Kalman dynamic hedge cointegration test passes
...
order book signals improve execution placement for crypto majors with $29{,}952$ simulated
parent orders on event-level Binance L2 data. They do not: aggressive crossing is cheapest
across all four symbols, the signal correlates $+0.03$ with the per-order advantage of posting over crossing, and a perfect-foresight oracle would save $1.4$ bps so the opportunity exists
but no contemporaneous feature forecasts it, and the same null holds when the simulation is rerun across all fifty symbols. Third, the candidate market-neutral reversion
...
surviving result ends up cutting against itself. A cointegration or mean-reversion screen should
be trusted only after it fails on a placebo built to
...
The relevant literatures are pairs trading, market microstructure, and cryptocurrency market
efficiency. The two screens we audit come from the cointegration branch of the pairs trading
literature. The order flow tools we apply to crypto L2 data are standard market microstructure.
The crypto market efficiency literature is the context for the headline negative result.
...
The toxicity program of Easley, L\'opez de Prado, and O'Hara~\cite{easley2012vpin} provides the
main longer-horizon order-flow benchmark, but Andersen and
...
data predicts at subsecond horizons, which is shorter than our hourly bars and consistent
...
Relative to this literature, we add three empirical checks. First, the matched placebo audit of
two standard cointegration screens (the result that an ADF test on Kalman
...
Second, the direct measurement of L2 execution value on crypto majors with $29{,}952$
simulated parent orders, with a perfect-foresight oracle as the upper bound, is, to our
knowledge, a direct crypto-market test of this execution-value question. Third, our
delisting stress test reinjects the LUNA, UST, and FTT collapses and shows that the structural
break circuit breaker preserves roughly $96\%$ of monthly Sharpe at the $8\%$/year breakage
rate consistent with historical delistings; unlike a complete point-in-time universe, it is a
...
symbols, over 2024. We analyze these venues separately because the venue gap matters: the spread strategy trades Binance.US, a
...
All reported inference uses Newey-West HAC standard
...
window only. Every positive claim is checked against a matched placebo, a random walk,
...
ratio~\cite{bailey2014dsr} (Appendix~\ref{app:dsr}), which discounts the Sharpe ratio (a strategy's mean return divided by its return standard deviation) for the number of configurations searched. Appendix~\ref{sec:appendix} gives self-contained definitions of every method and metric used in this paper, for readers unfamiliar with the full toolkit.
...
a placebo it should fail, and that neither carries the information it seems to. The rolling $z$ score is a standard entry signal and the
...
and approximately so here on the frozen parameter test window~\cite{harvey1989structural}. An ADF test on a nearly white
...
The placebo tests identify the problem. We ran the identical screen on series that cannot be cointegrated:
...
gap is essentially zero at every cadence, while the clean Engle-Granger test's gap is large and
...
posts a Sharpe near $4.75$ on our data still posts about $2.2$ on random-pair placebos and $2.4$ on
...
contemporaneous information about price formation, but none of it is monetized at the horizons
tested here. That null holds for both forecasting and execution.
...
imbalance~\cite{contkukanovstoikov2014} explains subsecond midprice moves better than trade flow does (contemporaneous incremental $R^2$
...
yet the one-second-ahead forecast $R^2$, taking trade and book OFI together, stays under $0.5\%$ for every one of the $50$ symbols
...
quote liquidity withdrawal (median $90\%$).\footnote{The only symbol whose forecast $R^2$ rises materially with horizon is USDC
...
The execution and size to impact results extend the same way. One result is weaker: rerunning the order
level execution simulation on all fifty symbols leaves the null intact: the signal-timed rule is
more expensive than aggressive crossing in every one of them (median $0.4$ bps worse) and on the whole loses to a
random post or cross. On the thinnest names a perfect foresight oracle could save $4$ to $5$ bps,
several times the $1.4$ on the four majors, yet the contemporaneous signal correlates at most $+0.07$ with
...
where institutional sized trades are only $0.2\%$ of orders, roughly an order of magnitude rarer than on the majors, and two
symbols (NMR and SAND) show a negative institutional estimate. Trade size proxies information where there is enough institutional
...
and all 2024 market regimes. The same decay holds across the full top 50 universe (Section~\ref{sec:real}). The information half-life is seconds; no tradable forecast
horizon survives.}
\label{fig:ofi-decay}
\end{figure}
...
\caption{Cancellations dominate trades in best-level liquidity withdrawal: $77$ to $90\%$ (about $77$ to $80\%$ for BTC, ETH, SOL; $90\%$ for AVAX) of
best-level size reductions are cancels, invisible to trade-only microstructure analysis
(BTC/ETH/SOL/AVAX, 2024).}
...
crash. Table~\ref{tab:exec-symbol-breakdown} confirms the pooled result holds per symbol and per
order size.
...
The execution simulation also gives an empirical cost estimate. Walking the real book puts execution costs about $17$
to $23\%$ below the flat 5 bps slippage component the earlier backtests assumed. Cheaper costs do
not rescue the strategy.

The efficiency reading is scoped to the liquid core. The Level 2 tests cover the full top 50 for the order flow, cancellation, and execution nulls,
and the four megacap symbols for the size to impact table, all on the global venue. The thin Binance.US microcaps where the
...
The remaining strategy is a market-neutral, diversified mean-reversion book in crypto pair
spreads. Its tradable magnitude is uncertain, it exists only without a stop, and a literal
backtest of it inflates spectacularly. We start with the limits, since they matter more than the
headline number.
...
(Binance naive $2.18 \to$ HAC $1.4$) together center the honest figure at $\sim$$1.0$.}
...
rare), and they posted a monthly Sharpe of $1.60$, with a HAC hourly annualized Sharpe of $0.95$ whose $95\%$
confidence interval $[-0.21, 1.89]$ spans zero. So the preregistered test confirms the inflation and gives no significant
...
breaks are drawn independently, which understates the way breaks cluster in market-wide selloffs.
The circuit breaker, not diversification alone, is what bounds the delisting tail,
...
There is a real, market-neutral reversion signal in crypto pair spreads that can be selected by
reversion speed. Whether it is worth anything after realistic costs, capacity, and selection is
unresolved, and the most honest single number, a monthly Sharpe near $1.0$, is an estimate that
a literal backtest does not cap from above.
...
stands.
\item \textbf{Microstructure coverage.} The book-OFI decay, cancellation, and execution results cover the full top 50 universe, over
...
only the next few seconds, not a horizon monetized in our tests. The one market-neutral reversion effect
...
The main result is methodological rather than tradable: a cointegration or mean-reversion screen
should fail on a matched placebo built to reproduce the claim if the claim is mechanical. For
crypto the evidence points to microstructure efficiency in the liquid majors at
